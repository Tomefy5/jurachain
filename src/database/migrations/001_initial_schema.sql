...
-- Description: Initial database schema for JusticeAutomation platform
-- Created: 2024-11-02

-- This migration creates the complete initial schema for the JusticeAutomation platform
-- It includes all tables, types, indexes, triggers, and RLS policies
...
CREATE INDEX IF NOT EXISTS idx_parties_document_id ON parties(document_id);
...
CREATE INDEX IF NOT EXISTS idx_risk_assessments_clause_id ON risk_assessments(clause_id);
CREATE INDEX IF NOT EXISTS idx_suggestions_risk_assessment_id ON suggestions(risk_assessment_id);
...
DROP POLICY IF EXISTS "Users can update own documents" ON legal_documents;
DROP POLICY IF EXISTS "Users can view parties of accessible documents" ON parties;
DROP POLICY IF EXISTS "Users can manage parties of own documents" ON parties;
DROP POLICY IF EXISTS "Users can view clauses of accessible documents" ON clauses;
DROP POLICY IF EXISTS "Users can manage clauses of own documents" ON clauses;
